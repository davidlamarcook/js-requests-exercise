...
const ohMy = () => {
    ...
    console.log(res)    
        let animalsContainer = document.querySelector('body')
...
const repeatMyParam = () => {
    axios.get('http://localhost:3000/repeat/go-lakers').then((res) => {
        console.log(res.data)
        let repeatParamText = document.getElementById('repeat-text')
        repeatParamText.textContent = res.data
        //repeatParamText.style.display = 'block';
    })
}
...
let sendQueryBtn = document.getElementById('query-button')
sendQueryBtn.addEventListener('click', makeQueryRequest);

//Not quite grasping this concept and why this example we use query-test? but I am seeing online other things are used and not sure what is the correct route to take?
...
/*
    ...
    1: Send no queries on the URL -- what happened? 
        //Nothing logs to show in the console 
    2: Send more than 1 query on the URL -- what happened? 
*/      //message prints saying 'You sent more than 1 query!'
